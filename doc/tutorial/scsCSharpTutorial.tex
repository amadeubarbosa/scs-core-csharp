...
Este documento é um tutorial básico sobre a criação de componentes no modelo SCS
 v1.3.0. Não serão encontradas aqui explicações sobre o modelo, as quais
 encontram-se em documentos específicos. Também não será abordado o uso de
 ...
 outros documentos.
A implementação do SCS C# versão 1.3.0 utiliza o .Net Framework 3.5 e IIOP.NET
 1.9.1. Este documento assume que o leitor é familiarizado a conceitos de
...
A classe IiopChannel possui dois contrutores. Para criar um canal apenas cliente
 é utilizado o construtor sem parâmetros. Já para criar um canal que aceita
 receber requisições, é necessário utilizar o construtor que possui a porta do
...
O identificador do componente é uma estrutura definida em IDL (scs.idl) chamada
 ComponentId, é representada por uma \emph{struct} que possui os seguintes
 campos:
...
O componente SCS é representado pela interface \emph{ComponentContext}. A classe
 \empth{DefaultComponentContext} é uma implementação padrão da interface, ela
...
A classe \emph{DefaultComponentContext} está localizada em \emph{Scs.Core} e seu
 ...
 utilizar o método \empth{updateFacet}.
...
ComponentId componentId = new ComponentId("MyComponent", 1, 0, 0, "");
...
Para adicionar a faceta implementada acima no componente, deve-se chamar o
 método \emph{AddFacet}, que possui como parâmetros o nome, a interface e um
 objeto que representa a implementação da faceta. O uso desse método pode ser
 visto no Código~\ref{lst:addFacet}.
...
Para adicionar um receptáculo ao componente, deve-se utilizar o método
 \emph{addReceptacle}, que espera como parâmetros o nome, a interface esperada
 e um \emph{boolean} indicando se o receptáculo deve aceitar uma ou múltiplas
 conexões. O uso desse método pode ser visto no Código~\ref{lst:addReceptacle}.
...
A interface \empth{ComponentContext} fornece métodos para acessar tanto as
...
A classe \empth{Receptacle} que possui os metadados de um receptáculo, contém as
...
O \emph{XMLComponentBuilder} procura, por meio de reflexão, a classe que
 implementa a faceta, utilizando o nome
Para obter a classe que implementa a facetas descrita, o elemento facetImpl e o
 atributo assembly.
...
O Código~\ref{lst:hellomain} implementa a faceta \emph{IHello}, que conta com
 apenas um método, \emph{sayHello}. Além disso, realiza a criação do componente.
 O código utiliza o builder \emph{XMLComponentBuilder} onde o xml pode ser visto
 no Código~\ref{lst:helloidl}.
...
É possível encontrar necessidade de estender classes que implementam facetas.
 Por exemplo, a faceta \emph{IComponent}, contém métodos para gerenciar o ciclo
 de vida do componente, chamados \emph{startup} e \emph{shutdown}. Como a lógica
 desses métodos deve ficar a cargo do desenvolvedor da aplicação, suas
 implementações padrão são vazias. O Código~\ref{lst:ciclodevida} serve de
 exemplo.
