...
Este documento é um tutorial básico sobre a criação de componentes no modelo SCS
 ...
 outros documentos.
A implementação do SCS C\# versão 1.2.1\_3 utiliza o .Net
...
A classe OrbServices possui três métodos estáticos para a criação e registro de canais de comunicação IIOP. Para criar um canal apenas
 cliente é utilizado o método com assinatura sem parâmetros. Já para criar um canal que
 aceite receber requisições, é necessário utilizar o método que possui a
 porta do canal como parâmetro, ou o método que recebe um mapa de propriedades, definindo a propriedade referente à porta e/ou outras especificadas pela documentação do IIOP.NET.
...
\begin{labeledcode}[CSharp]{lifetime}{Implementação da faceta Hello sobrescrevendo o método InitializeLifetimeService}
