...
Este documento é um tutorial básico sobre a criação de componentes no modelo SCS
 v1.3.0\cite{SCS}. Não serão encontradas aqui explicações sobre o modelo, as
 ...
 outros documentos.
A implementação do SCS C\# versão 1.3.0 utiliza o .Net
 Framework 3.5\cite{NETFRAMEWORK} e IIOP.NET\cite{IIOPNET} 1.9.1. Este documento
...
A classe \emph{DefaultComponentContext} está localizada em \emph{Scs.Core} e seu
 ...
 utilizar o método \emph{updateFacet} que será explicado adiante.
...
O \emph{XMLComponentBuilder} procura, por meio de reflexão, a classe que
 implementa a faceta. O mecanismo utiliza o valor do elemento facetImpl e do
 atributo assembly para identificar a classe descrita no XML.
...
O Código~\ref{lst:hellomain} implementa a faceta \emph{IHello}, que conta com
 ...
 O código utiliza o builder \emph{XMLComponentBuilder}. O XML pode ser visto no
 Código~\ref{lst:xml}.
